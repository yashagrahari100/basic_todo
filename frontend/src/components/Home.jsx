...
function Home() {
    ...
    const [newTodo, setNewTodo] = useState("")
        ...
        const fetchtodos = async () => {
            try {
                ...
                const response = await axios.get("http://localhost:3000/todo/fetch", {
                    ...
                    // headers: {
                    //     "Content-Type": "application/json",//json fetch data in frontend
                    // }
                })
                console.log("Fetched Data: ", response.data.todos);
     ...
     // [] added empty dependency array for running the program once
    ...
    return (
        <div className=' my-10 bg-gray-100 max-w-lg lg:max-w-xl rounded-lg shadow-lg mx-8 sm:mx-auto p-6'>
            <h1 className=' mb-4 text-2xl font-semibold text-center'>Todo App</h1>
            ...
            {loading?(<div className='text-center justify-center'>
                <span className='textgray-500'>Loading...</span></div>):error?(<div className='text-center text-red-500 font-semibold'>{error}</div>):(
            ...
            <button onClick={() => logout()} className='mt-6 px-4 py-2 bg-red-500 text-white rounded-md hover:text-red-800 duration-500 mx-auto block'>Logout</button>
        </div>
    )
}
